...
// define Validator struct which contains a map of validation errors
type Validator struct {
	Errors map[string]string
}

// create a New() func that returns validator's instance with an empty errors map
func New() *Validator {
	return &Validator{Errors: make(map[string]string)}
}

// valid func returns true if errors map doesn't contain any entries
